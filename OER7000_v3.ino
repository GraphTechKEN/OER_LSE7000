...
//VID 0F0D
//PID 00C1
//C:\Program Files (x86)\Arduino\hardware\arduino\avr\boards.txt 内または、
//C:\Users\User\AppData\Local\Arduino15\packages\arduino\hardware\avr\1.8.7\boards.txt 内
// micro.vid.1=0x0F0D
// micro.pid.1=0x00C1
...
#define PIN_BRK_NOTCH_2 3
#define PIN_BRK_NOTCH_3 4
#define PIN_BRK_NOTCH_4 5
#define PIN_BRK_NOTCH_5 6 
#define PIN_BRK_NOTCH_6 1
...
void setup() {
  ...
  pinMode(PIN_BRK_NOTCH_2, INPUT_PULLUP);
  pinMode(PIN_BRK_NOTCH_3, INPUT_PULLUP);
  pinMode(PIN_BRK_NOTCH_4, INPUT_PULLUP);
  pinMode(PIN_BRK_NOTCH_5, INPUT_PULLUP);
  pinMode(PIN_BRK_NOTCH_6, INPUT_PULLUP);
...
uint8_t read_Break() {
  ...
  bool PIN_BRK_2 = !digitalRead(PIN_BRK_NOTCH_2);
  bool PIN_BRK_3 = !digitalRead(PIN_BRK_NOTCH_3);
  bool PIN_BRK_4 = !digitalRead(PIN_BRK_NOTCH_4);
  bool PIN_BRK_5 = !digitalRead(PIN_BRK_NOTCH_5);
  bool PIN_BRK_6 = !digitalRead(PIN_BRK_NOTCH_6);
...
if(PIN_BRK_6 ){
  //B1
  if (mc_notch == 0 && PIN_BRK_1 && PIN_BRK_2 && PIN_BRK_3 && PIN_BRK_4 && !PIN_BRK_5 && PIN_BRK_7 && PIN_BRK_8) {
    ...
    //B3
  } else if (mc_notch == 0 && PIN_BRK_1 && PIN_BRK_3 && PIN_BRK_5 && PIN_BRK_6 && PIN_BRK_7 && PIN_BRK_8) {
    if (modeBVE || Yokusoku_Mode) {
      _brk_notch = 3;
    } else {
      _brk_notch = 4;
    ...
    //B4
  } else if (mc_notch == 0 && !PIN_BRK_1 && !PIN_BRK_2 && !PIN_BRK_3 && !PIN_BRK_4 && !PIN_BRK_5 && PIN_BRK_6 && PIN_BRK_7 && PIN_BRK_8) {
    if (modeBVE || Yokusoku_Mode) {
      _brk_notch = 4;
    } else {
      _brk_notch = 5;
